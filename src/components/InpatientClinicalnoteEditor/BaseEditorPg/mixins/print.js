...
import { setPrintXmlList } from '@/utils/consultationFunctions.js'
...
let mixin = {
  ...
  methods: {
    async serialPrint(
      ...
      if (printStartIndex !== -1) {
        ...
        printList.forEach((el) => {
          ...
          let activatedPrintItem = {}

          // let activatedPrintItem = res.data.find(
          //   (ele) => el.id == ele.inpatEmrSetId
          // )
          let isPrint = activatedPrintItem.printed || true
    ...
    async consultationPrint(
      ...
      let xmlList = setPrintXmlList(xml)
